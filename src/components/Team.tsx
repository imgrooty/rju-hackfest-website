          ...
          {teams.map((team, teamIndex) => (
            <div key={teamIndex} className="space-y-6">
              ...
              <div className={`grid gap-4 ${
                teamIndex === 0 ? 'grid-cols-1 sm:grid-cols-2 grid-rows-2' : 'grid-cols-1 md:grid-cols-2'
              }`}>
                {team.members.map((member, memberIndex) => {
                  ...
                  return (
                    ...
                    >
                      <Card className={`h-full overflow-hidden border-none shadow-lg transition-all duration-500 ${
                        ...
                        <CardContent className="p-6">
                          <div className="flex flex-col md:flex-row gap-6">
                            <div className={`flex-shrink-0 transition-all duration-500 ${
                              isExpanded ? 'md:w-32 md:h-32' : 'w-16 h-16'
                            }`}>
                              <div className={`w-full h-full rounded-full overflow-hidden bg-hackathon-primary/20 flex items-center justify-center ${
                                isExpanded ? 'border-4 border-hackathon-primary/30' : ''
                              }`}>
                                {member.image ? (
                                  <img
                                    src={member.image}
                            ...
                            <div className="flex-1">
                              <h4 className={`font-bold transition-all duration-500 ${
                                isExpanded ? 'text-2xl mb-2' : 'text-xl'
                              }`}>
                                {member.name}
                              </h4>
